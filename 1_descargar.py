...
def listar_programas(page):
    """Abre el buscador de Programa y devuelve [(codigo, nombre), ...].

    La consulta filtra por un programa a la vez y el sistema se cuelga si se
    pide el municipio entero, asi que hay que recorrer direccion por
    direccion. Los codigos no estan en ninguna parte del script: se leen del
    propio dialogo 'Buscar Programa', que los lista con su nivel superior.
    Solo interesan las filas cuyo nivel superior es el POA (las direcciones);
    las que cuelgan de una direccion son sus programas internos.
    """
    c = page.locator("input[name=program]").first
    if not c.count():
        raise SystemExit("No existe el campo 'program' en la pantalla.")

    # El icono de lupa del campo abre el dialogo de busqueda.
    abierto = False
    for sel in ["input[name=program] ~ * [title*='uscar']",
                "input[name=program] ~ [title*='uscar']",
                "input[name=program] ~ * i.fa-search",
                "input[name=program] ~ * .fa-search"]:
        ...
        try:
            ...
            continue
    if not abierto:
        # Con el campo enfocado, F3 abre el buscador en eGob.
        c.click(timeout=10_000)
        page.keyboard.press("F3")

    dlg = page.locator("div.modal.in:has-text('Buscar Programa')").first
    try:
        dlg.wait_for(state="visible", timeout=20_000)
    except Exception:
        captura(page, "sin_buscador_programa")
        raise SystemExit("No se abrio el dialogo 'Buscar Programa': "
                         "revise sin_buscador_programa.png")

    vistos, orden = {}, []
    for pagina in range(1, 41):
        esperar_procesando(page)
        page.wait_for_timeout(800)
        filas = dlg.locator("tbody tr")
        for i in range(filas.count()):
            celdas = filas.nth(i).locator("td")
            if celdas.count() < 3:
                continue
            try:
                nivel = (celdas.nth(0).inner_text() or "").strip()
                codigo = (celdas.nth(1).inner_text() or "").strip()
                nombre = (celdas.nth(2).inner_text() or "").strip()
            except Exception:
                continue
            # nivel superior "2026 - POA 2026" => es una direccion
            if not codigo or "POA" not in nivel.upper():
                continue
            if codigo in vistos:
                continue
            vistos[codigo] = nombre
            orden.append((codigo, nombre))

        sig = dlg.locator("[title*='iguiente'], .pager-next, "
                          "button:has-text('>')").last
        try:
            if not sig.count() or not sig.is_enabled():
                break
            sig.click(timeout=8_000)
        except Exception:
            break

    for sel in ["button:has-text('CANCELAR')", "button:has-text('Cancelar')",
                "[data-dismiss=modal]", "button.close"]:
    ...
    log(f"   {len(orden)} direccion(es) en el POA")
    salida = [{"codigo": c,
               "buscar": re.sub(r"^\d{4}\.", "", c),
               "sugerencia": c,
               "nombre": n,
               "asignacion": None} for c, n in orden]
    destino = os.path.join(DATOS, "direcciones.json")
    os.makedirs(DATOS, exist_ok=True)
    with open(destino, "w", encoding="utf-8") as f:
        json.dump(salida, f, ensure_ascii=False, indent=2)
    log(f"   catalogo en {destino}")
    return salida


def direcciones_configuradas():
    """Devuelve el catalogo de programas del POA como lista de dicts.

    Cada entrada lleva:
      codigo      el del POA, "2026.3.6"
      buscar      lo que se escribe en el campo Programa, "3.6"
      sugerencia  el texto que hay que reconocer en el autocompletado
      nombre      para el log
      asignacion  asignacion inicial del POA, para cuadrar la descarga

    Se busca direcciones.json junto al script, luego datos/direcciones.json
    ...
    for ruta in (os.path.join(BASE, "direcciones.json"),
        ...
        for d in crudo:
            ...
            salida.append({
                "codigo": codigo,
                "buscar": d.get("buscar") or re.sub(r"^\d{4}\.", "", codigo),
                "sugerencia": d.get("sugerencia") or codigo,
                "nombre": d.get("nombre", ""),
                "asignacion": d.get("asignacion"),
            })
...
def filtrar(page, programa=None, sugerencia="DIRECCIÓN GENERAL"):
...
def descargar_partidas(page, sufijo=""):
    log("6. Descargando Partida XLS")
    nombre = f"partidas{sufijo}.xls" if sufijo else "partidas.xls"
...
def slug(texto):
    limpio = re.sub(r"[^A-Za-z0-9]+", "_", texto).strip("_").lower()
...
def descargar_por_direccion(page, direcciones, detalle=False, todas=False):
    ...
    direccion a la vez. Entre una y otra hay que volver a abrir la consulta:
    reutilizar la pantalla arrastra los identificadores de la carga anterior y
    eGob responde con el aviso de registros inexistentes.
    """
    hechos, fallidos, cuadre = [], [], []
    for i, d in enumerate(direcciones, 1):
        codigo = d["codigo"]
        etiqueta = d.get("nombre") or codigo
        log("")
        log(f"=== {i}/{len(direcciones)}  {codigo}  {etiqueta}")
        try:
            if i > 1:
                abrir_consulta(page)
            filtrar(page, d["buscar"], d["sugerencia"])
            ...
            captura(page, f"error_{slug(codigo)}")
        except Exception as e:
            log(f"   ! {codigo}: {e}")
            fallidos.append((codigo, str(e)))
            captura(page, f"error_{slug(codigo)}")
    ...
    log("")
    log(f"Descargadas {len(hechos)} de {len(direcciones)} direcciones")
    for codigo, motivo in fallidos:
        log(f"   pendiente {codigo}: {motivo[:90]}")

    ok = [c for c in cuadre if c]
    ...
    if malos:
        ...
        for c in malos:
            log(f"   {c['codigo']:>10}  POA {c['esperado']:>15,.2f}"
                ...
                f"   dif {c['diferencia']:>13,.2f}")
    elif ok:
        log(f"   las {len(ok)} direcciones cuadran con la asignacion del POA")

    total = sum(c["obtenido"] for c in ok)
    if total:
        log(f"   asignacion inicial descargada: {total:,.2f}")
    return hechos


def comprobar_asignacion(ruta, direccion):
    """Compara la asignacion inicial del XLS con la del catalogo del POA.

    Es la unica forma de saber que la descarga trajo la direccion completa:
    si el filtro se aplico a medias o la seleccion quedo corta, el total no
    cuadra y la descarga hay que repetirla.
    """
...
def main():
    ...
    ap.add_argument("--listar-direcciones", action="store_true",
                    help="Leer el buscador de Programa y guardar "
                         "datos/direcciones.json, sin descargar nada")
    ap.add_argument("--por-direccion", action="store_true",
                    help="Bajar un Partida XLS por cada dirección del POA "
                         "(la consulta completa cuelga el sistema)")
    a = ap.parse_args()
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            if a.listar_direcciones:
                for d in listar_programas(page):
                    log(f"   {d['codigo']:>12}  {d['nombre']}")
                log(f"Listo en {time.time() - inicio:.0f} s")
                return

            if a.por_direccion:
                direcciones = direcciones_configuradas()
                if not direcciones:
                    log("   sin catálogo previo, lo leo del buscador")
                    direcciones = listar_programas(page)
                    abrir_consulta(page)
                if not direcciones:
                    raise SystemExit(
